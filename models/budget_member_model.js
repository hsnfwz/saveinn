...
//     const { rows: rows3 } = await pool.query(poolQuery3, [username, email, salt, hash, firstName, lastName, employmentPosition, postalCode]);
...
//     const { rows: rows3 } = await pool.query(poolQuery3, [email, budgetMemberId]);
...
//     const { rows: rows3 } = await pool.query(poolQuery3, [username, budgetMemberId]);
...
//     const { rows } = await pool.query(poolQuery, [salt, hash, budgetMemberId]);
