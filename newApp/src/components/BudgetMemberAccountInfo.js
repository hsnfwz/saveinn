...
function BudgetMemberAccountInfo() {
  const [showEditModal, setShowEditModal] = useState(false);

  const [budgetMemberRecord, setBudgetMemberRecord] = useState(undefined);
  ...
  async function handleRefresh() {
    const budgetMemberId = location.pathname.split('/')[2];

    try {
      const endpoint = `http://localhost:8080/budgetMember/${budgetMemberId}`;
  ...
  async function handleEdit() {
    try {
      const endpoint = `http://localhost:8080/budgetMember/${budgetMemberRecord.id}`;

      const body = {
        userId: budgetMemberRecord.id,
        firstName,
        lastName,
      }
          ...
          <Row>
            ...
            <Row>
              <Col className='d-flex align-items-center justify-content-center py-2'>
                <h3 className='me-1'>{ budgetMemberRecord.firstName }</h3>
                <h3>{ budgetMemberRecord.lastName }</h3>
              </Col>
            </Row>
            <Row>
              <Col className='d-flex justify-content-center mt-2'>
                <Button className="btn btn-secondary blueBtns mx-2" style={{fontWeight:"normal"}} onClick={async () => {
                  setFirstName(budgetMemberRecord.firstName);
                  setLastName(budgetMemberRecord.lastName);
